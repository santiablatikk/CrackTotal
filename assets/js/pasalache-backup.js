...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let selectedGameMode = 'solo'; // Default to solo mode
    
    // Game mode cards event listeners
    ...
    if (startGameButton) {
        startGameButton.addEventListener('click', function(e) {
            ...
            const selectedDifficulty = selectedCard ? selectedCard.getAttribute('data-difficulty') : 
                                     selectedButton ? selectedButton.getAttribute('data-difficulty') : 'normal';
    ...
    function showMultiplayerModal() {
        const rulesModal = document.getElementById('gameRulesModal');
        const multiplayerModal = document.getElementById('multiplayerModal');
        
        if (rulesModal) rulesModal.style.display = 'none';
        if (multiplayerModal) multiplayerModal.style.display = 'block';
    }
    
    function startSoloGame(difficulty) {
        // Original solo game logic
        console.log('Starting solo game with difficulty:', difficulty);
        
        // Hide rules modal and start game
        const rulesModal = document.getElementById('gameRulesModal');
        if (rulesModal) rulesModal.style.display = 'none';
        
        // Set multiplayer game mode
        if (window.pasalacheMultiplayer) {
            window.pasalacheMultiplayer.gameMode = 'solo';
        }
    ...
    // Multiplayer modal event listeners
    const createRoomButton = document.getElementById('createRoomButton');
    const joinRoomButton = document.getElementById('joinRoomButton');
    const backToGameModeButton = document.getElementById('backToGameModeButton');
    ...
    if (createRoomButton) {
        createRoomButton.addEventListener('click', async function(e) {
            ...
            try {
                ...
                const selectedCard = document.querySelector('.difficulty-card.active');
                const selectedButton = document.querySelector('.difficulty-btn.active');
                const difficulty = selectedCard ? selectedCard.getAttribute('data-difficulty') : 
                                 selectedButton ? selectedButton.getAttribute('data-difficulty') : 'normal';
                
                // Create room
                const roomId = await window.pasalacheMultiplayer.createRoom(difficulty);
                showWaitingRoom(roomId);
                
            } catch (error) {
                console.error('Error creating room:', error);
                alert('Error creando la sala. Intenta nuevamente.');
            }
        });
    }
    ...
    if (joinRoomButton) {
        joinRoomButton.addEventListener('click', async function(e) {
            ...
            try {
                await window.pasalacheMultiplayer.joinRoom(roomCode);
                showWaitingRoom(roomCode);
                
            } catch (error) {
                console.error('Error joining room:', error);
                alert('Error uniéndose a la sala: ' + error.message);
            }
        });
    }
    
    if (backToGameModeButton) {
        backToGameModeButton.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    function showWaitingRoom(roomId) {
        ...
        if (multiplayerModal) multiplayerModal.style.display = 'none';
        if (waitingRoomModal) waitingRoomModal.style.display = 'block';
    ...
    function updatePlayersDisplay() {
        ...
        if (!playersContainer || !window.pasalacheMultiplayer.roomData) return;
        
        const players = Object.values(window.pasalacheMultiplayer.roomData.players);
    ...
    if (copyRoomCodeButton) {
        copyRoomCodeButton.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    if (leaveRoomButton) {
        leaveRoomButton.addEventListener('click', function(e) {
            ...
            window.pasalacheMultiplayer.leaveRoom();
            
            // Hide waiting room modal and show rules modal
            const waitingRoomModal = document.getElementById('waitingRoomModal');
            const rulesModal = document.getElementById('gameRulesModal');
        ...
        // Update multiplayer UI when room data changes
    if (window.pasalacheMultiplayer) {
        const originalUpdateGameState = window.pasalacheMultiplayer.updateGameState;
        window.pasalacheMultiplayer.updateGameState = function() {
            originalUpdateGameState.call(this);
            updatePlayersDisplay();
        };
    }
