...
console.log('Ranking Mentiroso script loaded and Firebase initialized for real-time updates');
...
// --- Cargar Ranking Global de Mentiroso ---
function loadRanking() {
    ...
    try {
        ...
        // Consulta: Ordenar por Victorias de Mentiroso DESC, luego Partidas Jugadas DESC
        const q = query(usersRef, 
                        orderBy("stats.mentiroso.wins", "desc"), 
                        orderBy("stats.mentiroso.played", "desc"), 
                        limit(50)); 

        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            console.log("Mentiroso ranking data received/updated");
            ...
            querySnapshot.forEach((doc) => {
                ...
                const totalScore = userData.stats?.mentiroso?.score || 0;
                const matchesPlayed = userData.stats?.mentiroso?.played || 0;
                const wins = userData.stats?.mentiroso?.wins || 0;
                const losses = (matchesPlayed - wins) || 0;

                // Solo mostrar jugadores que han jugado al menos una partida de Mentiroso
                if (matchesPlayed === 0) {
...
function loadHistory() {
    ...
    try {
        ...
        const q = query(matchesRef, 
                       where("gameType", "==", "mentiroso"),
                       orderBy("timestamp", "desc"), 
                       limit(20));

        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            console.log("Mentiroso history data received/updated");
            historyList.innerHTML = '';

            if (querySnapshot.empty) {
                historyList.innerHTML = '<p>No hay partidas de Mentiroso registradas en el historial.</p>';
                return;
            }
            ...
            querySnapshot.forEach((doc) => {
                ...
                matchEntry.innerHTML = `
                    ...
                    <div class="match-card-footer">
                        ...
                        <span class="detail-item" title="Categoría">
                            <i class="fas fa-tag"></i> ${matchData.category || 'General'}
                        </span>
                    </div>
                `;
            ...
            console.error("Error al escuchar el historial de Mentiroso: ", error);
            historyList.innerHTML = '<p>Error al cargar el historial en tiempo real. Inténtalo de nuevo más tarde.</p>';
        });

    } catch (error) {
        console.error("Error al configurar la consulta del historial de Mentiroso: ", error);
